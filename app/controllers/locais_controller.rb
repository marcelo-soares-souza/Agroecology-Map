...
class LocaisController < ApplicationController
  ...
  def index
    @locais = if params[:usuario_id]
      Local.where(usuario_id: @usuario.id).includes(:experiencia_agroecologicas).load_async.sort_by(&:updated_at).reverse
    else
      Local.all.includes(:experiencia_agroecologicas).load_async.sort_by(&:updated_at).reverse
    end
  end
